...
def aplicarCambiosSobrescritura(archivos_con_contenido, rutaBase, accionOriginal, paramsOriginal):
    ...
    *** MODIFICADO: Crea .gitkeep al crear directorios. ***
    """
    ...
    exito_accion = False # Para rastrear si la acción principal tuvo éxito
    mensaje_error = None # Para almacenar errores
    ...
    if accionOriginal in ["eliminar_archivo", "crear_directorio"]:
        ...
        if not targetRel:
            mensaje_error = f"Missing target parameter ('archivo' or 'directorio') for {accionOriginal}."
            log.error(f"{logPrefix} {mensaje_error}")
            return False, mensaje_error
        targetAbs = _validar_y_normalizar_ruta(targetRel, rutaBaseNorm, asegurar_existencia=False)
        if targetAbs is None:
            mensaje_error = f"Invalid or unsafe path provided for {accionOriginal}: '{targetRel}'"
            log.error(f"{logPrefix} {mensaje_error}")
            return False, mensaje_error
        ...
        if accionOriginal == "eliminar_archivo":
            ...
            if os.path.exists(targetAbs):
                try:
                    if os.path.isfile(targetAbs) or os.path.islink(targetAbs):
                        ...
                        try:
                            os.rmdir(targetAbs) # Intenta borrar si está vacío
                        ...
                        return False, err
                    exito_accion = True # Marcamos éxito si la eliminación funcionó
                except Exception as e:
                    mensaje_error = f"Error deleting '{targetRel}': {e}"
                    log.error(f"{logPrefix} {mensaje_error}", exc_info=True)
                    return False, mensaje_error
            else:
                log.warning(f"{logPrefix} Target '{targetRel}' not found for deletion. Considering successful.")
                exito_accion = True # Consideramos éxito si no existía
            return exito_accion, mensaje_error # Devolver estado
            ...
            if os.path.exists(targetAbs):
                if os.path.isdir(targetAbs):
                    ...
                    exito_accion = True # Ya existe, consideramos éxito
                else:
                    mensaje_error = f"Path '{targetRel}' exists but is not a directory. Cannot create directory."
                    log.error(f"{logPrefix} {mensaje_error}")
                    return False, mensaje_error
            else:
                try:
                    ...
                    exito_accion = True # La creación tuvo éxito
                except Exception as e:
                    mensaje_error = f"Error creating directory '{targetRel}': {e}"
                    log.error(f"{logPrefix} {mensaje_error}", exc_info=True)
                    return False, mensaje_error

            # --- INICIO: AÑADIR .gitkeep ---
            if exito_accion: # Solo si el directorio se creó o ya existía correctamente
                ...
                if not os.path.exists(gitkeep_path): # Crear solo si no existe
                    try:
                        ...
                        # No consideramos esto un fallo fatal de la acción principal, pero sí un warning
                        log.warning(f"{logPrefix} No se pudo crear .gitkeep en '{targetRel}': {e_gk}")
                else:
                    log.debug(f"{logPrefix} Archivo .gitkeep ya existente en '{targetRel}'.")
            # --- FIN: AÑADIR .gitkeep ---

            return exito_accion, mensaje_error # Devolver estado de la creación del dir
    ...
    if not isinstance(archivos_con_contenido, dict):
         ...
         log.error(f"{logPrefix} {err}")
         return False, err
    # Permitir dict vacío si la acción original era crear_directorio o eliminar_archivo
    if not archivos_con_contenido and accionOriginal not in ["eliminar_archivo", "crear_directorio"]:
        ...
        log.error(f"{logPrefix} {err}")
        return False, err
    elif not archivos_con_contenido:
         log.info(f"{logPrefix} No content in 'archivos_con_contenido', which is expected for action '{accionOriginal}'.")
         # Si llegamos aquí para crear/eliminar, la acción ya se completó arriba. Retornar True.
         return True, None
