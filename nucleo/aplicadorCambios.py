...
MOJIBAKE_REPLACEMENTS = {
    b'\xc3\xa1'.decode('latin-1', errors='ignore'): "á",  # á -> á
    b'\xc3\xa9'.decode('latin-1', errors='ignore'): "é",  # é -> é
    b'\xc3\xad'.decode('latin-1', errors='ignore'): "í",  # í -> í
    b'\xc3\xb3'.decode('latin-1', errors='ignore'): "ó",  # ó -> ó
    b'\xc3\xba'.decode('latin-1', errors='ignore'): "ú",  # ú -> ú
    b'\xc3\xbc'.decode('latin-1', errors='ignore'): "ü",  # ü -> ü
    ...
    b'\xc3\xb1'.decode('latin-1', errors='ignore'): "ñ",  # ñ -> ñ
    b'\xc3\x91'.decode('latin-1', errors='ignore'): "Ñ",  # Ã‘ -> Ñ
    b'\xc2\xa1'.decode('latin-1', errors='ignore'): "¡",  # ¡ -> ¡
    b'\xc2\xbf'.decode('latin-1', errors='ignore'): "¿",  # ¿ -> ¿
    b'\xc2\xaa'.decode('latin-1', errors='ignore'): "ª",  # ª -> ª
    b'\xc2\xba'.decode('latin-1', errors='ignore'): "º",  # º -> º
    b'\xc2\xab'.decode('latin-1', errors='ignore'): "«",  # « -> «
    b'\xc2\xbb'.decode('latin-1', errors='ignore'): "»",  # » -> »
        ...
        for mojibake, correct in MOJIBAKE_REPLACEMENTS.items():
            if isinstance(temp_contenido_mojibake, str):
                ...
                if new_temp_contenido_mojibake != temp_contenido_mojibake:
                    ...
                    log.debug(f"Reemplazado Mojibake: {repr(mojibake_str)} -> {repr(correct)} en '{rutaRel}'")
