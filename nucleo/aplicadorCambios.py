...
def aplicarCambio(accion, rutaBase):
    ...
    try:
            ...
            if isinstance(codigoNuevo, str):
                ...
                try:
                    ...
                    if contenidoModificado == contenidoOriginal:
                        # Advertir si 'buscar' no se encontró o el reemplazo no hizo nada
                        log.warning(f"{logPrefix} Texto a buscar no encontrado o el reemplazo no cambió el contenido en {archivoRel}. Revisar 'buscar': '{buscar[:100]}...'")
                        # Considerar éxito para no bloquear, pero necesita revisión manual.
                        # Podría ser un error si se esperaba un cambio sí o sí.
                        return True # Devolver True para no fallar el ciclo completo.
                    else:
        ...
        # <<< INICIO: Nueva Acción mover_codigo >>>
            ...
            try:
                ...
                # Quitar espacio final si existe, añadir nueva línea, código, nueva línea
                contenidoDestinoModificado = contenidoDestino.rstrip() + "\n\n" + codigoAMover.strip() + "\n"

                # --- Eliminar código del Origen (reemplazo simple) ---
                # ¡CUIDADO! Esto es sensible si el código aparece varias veces.
                # La IA debe ser instruida para proporcionar fragmentos únicos/completos.
                ...
                if contenidoOrigenModificado == contenidoOrigen:
                    # Esto no debería pasar si la comprobación 'in' anterior tuvo éxito.
                    # Podría indicar problemas con newlines o caracteres invisibles.
        ...
        # <<< FIN: Nueva Acción mover_codigo >>>
            ...
            origenAbs = _validar_y_normalizar_ruta(origenRel, rutaBaseNorm, asegurar_existencia=True)
            if not origenAbs: return False
            destinoAbs = _validar_y_normalizar_ruta(destinoRel, rutaBaseNorm, asegurar_existencia=False) # Destino no debe existir
            if not destinoAbs: return False

            # Asegurar que origen es un archivo
            if not os.path.isfile(origenAbs):
                 log.error(f"{logPrefix} El origen a mover no es un archivo: {origenAbs}")
                 return False
            # Asegurar que destino no exista (para evitar sobrescribir)
            if os.path.exists(destinoAbs):
            ...
            archivoRel = detalles.get("archivo")
            contenido = detalles.get("contenido", "") # Default a vacío si no se provee
            ...
            archivoRel = detalles.get("archivo")
            archivoAbs = _validar_y_normalizar_ruta(archivoRel, rutaBaseNorm, asegurar_existencia=False) # No asegurar, puede que ya no exista
            if not archivoAbs: return False # Si la ruta es inválida sí fallar
            ...
            if os.path.exists(dirAbs):
                if os.path.isdir(dirAbs):
                     ...
                     return True # Éxito si ya existe como directorio
                else:
                     log.error(f"{logPrefix} Existe un archivo con el mismo nombre que el directorio a crear: {dirAbs}")
                     return False

            try:
                os.makedirs(dirAbs, exist_ok=True) # exist_ok=True es seguro
            ...
            # Devolver True porque la "acción" (no hacer nada) se completó como se esperaba.
            return True

        else:
            # Esto no debería ocurrir si parsearSugerencia funciona bien
