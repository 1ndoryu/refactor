...
def leerArchivos(listaArchivos, rutaBase):
    ...
    if archivosFallidos:
         log.warning(f"{logPrefix} No se pudieron leer {len(archivosFallidos)} archivos: {archivosFallidos[:5]}...")
...
# --- PASO 1: Obtener Decisión (Prompt Modificado para incluir Estructura) ---
# <<< --- MODIFICADO: Añadir parámetro proyecto_estructura_texto --- >>>
def obtenerDecisionRefactor(contextoCodigoCompleto, historialCambiosTexto=None, proyecto_estructura_texto=None):
    """
    ...
    promptPartes = []
    # <<< --- MODIFICADO: Mencionar la estructura en la descripción --- >>>
    promptPartes.append("Eres un asistente experto en refactorización de código PHP/JS (WordPress). Tu tarea es analizar TODO el código fuente, la ESTRUCTURA del proyecto y el historial, y proponer UNA ÚNICA acción de refactorización PEQUEÑA, SEGURA y ATÓMICA.")
    ...
    promptPartes.append("Considera el historial para NO repetir errores, NO deshacer trabajo anterior y mantener la consistencia. Usa la ESTRUCTURA del proyecto proporcionada para entender las rutas y la organización general al decidir acciones como mover código o crear archivos.") # <<< --- MODIFICADO: Indicar uso de estructura

    # <<< --- NUEVO: Añadir Estructura al Prompt --- >>>
    promptPartes.append("\n--- ESTRUCTURA ACTUAL DEL PROYECTO (para referencia de rutas y organización) ---")
    if proyecto_estructura_texto and proyecto_estructura_texto != "[Error al generar estructura]":
        promptPartes.append(proyecto_estructura_texto)
    elif proyecto_estructura_texto == "[Error al generar estructura]":
         promptPartes.append("(Error: No se pudo generar la estructura del proyecto para este análisis.)")
    else:
        promptPartes.append("(Nota: No se proporcionó la estructura del proyecto para este análisis.)")
    ...
    promptPartes.append("2.  **DESCRIBE CLARAMENTE**: En `descripcion`, sé MUY específico para un mensaje de commit útil (ej: 'Refactor(Seguridad): Añade isset() a $_GET['param'] en archivo.php linea 55', 'Refactor(Clean): Elimina función duplicada viejaFuncion() de utils_old.php', 'Refactor(Org): Mueve función auxiliar miHelper() de main.php a helpers/ui.php').")
    promptPartes.append("3.  **DETALLA PARÁMETROS SIN AMBIGÜEDAD**: En `parametros_accion`, incluye TODA la información necesaria para que OTRO proceso (Paso 2) realice el cambio SIN DUDAS. Usa rutas RELATIVAS al raíz del proyecto (como se ven en la ESTRUCTURA).") # <<< --- MODIFICADO: Referencia a estructura
    promptPartes.append("    -   `mover_funcion`/`mover_clase`: `archivo_origen`, `archivo_destino`, `nombre_funcion`/`nombre_clase`. **IMPORTANTE**: Indica si la función/clase debe ser **BORRADA** del origen (`eliminar_de_origen`: true/false).")
    ...
    promptPartes.append("    -   `crear_archivo`: `archivo` (ruta completa relativa, consistente con la ESTRUCTURA), `proposito_del_archivo` (breve, ej: 'Clase para manejar la API externa X').") # <<< --- MODIFICADO: Referencia a estructura
    promptPartes.append("    -   `eliminar_archivo`: `archivo` (ruta relativa).")
    promptPartes.append("    -   `crear_directorio`: `directorio` (ruta relativa, consistente con la ESTRUCTURA).") # <<< --- MODIFICADO: Referencia a estructura
    promptPartes.append("4.  **LISTA ARCHIVOS RELEVANTES COMPLETAMENTE**: En `archivos_relevantes`, incluye **TODAS** las rutas relativas de archivos que el Paso 2 **NECESITARÁ LEER** para ejecutar la acción (origen, destino, archivos que usan lo movido si aplica y es fácil de determinar, etc.). ¡Esto es CRUCIAL y debe ser preciso! Usa rutas como se ven en la ESTRUCTURA.") # <<< --- MODIFICADO: Referencia a estructura
...
```""")
    ...
    promptPartes.append("\nRecuerda: JSON estricto, UNA acción pequeña y segura, parámetros DETALLADOS, razonamiento CLARO, archivos relevantes COMPLETOS. Usa la ESTRUCTURA proporcionada como guía.")
    ...
    # Loguear el prompt completo a nivel TRACE si tuvieramos ese nivel
    # logging.log(logging.TRACE, f"{logPrefix} Prompt completo:\n{promptCompleto}")
    ...
    try:
        respuesta = modelo.generate_content(
            ...
            generation_config=genai.types.GenerationConfig(
                ...
                # max_output_tokens=8192 # Ajustar si se espera contenido muy largo
                ),
             safety_settings={
                 'HATE': 'BLOCK_ONLY_HIGH',
                 'HARASSMENT': 'BLOCK_ONLY_HIGH',
