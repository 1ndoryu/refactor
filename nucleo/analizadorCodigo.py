...
def obtenerDecisionRefactor(contextoCodigoCompleto, historialCambiosTexto=None):
    ...
    promptPartes.append(
        "Analiza el historial reciente para evitar repetir errores, acciones ineficaces o entrar en bucles de refactorización.")
    ...
    promptPartes.append(
        "4.  **Impacto Esperado:** Menciona brevemente cómo esta acción mejora el código (legibilidad, mantenibilidad, rendimiento, etc.).")
    ...
    promptPartes.append(
        "1.  **RAZONAMIENTO COMPLETO:** Incluye el proceso de razonamiento detallado en el campo `razonamiento`.")
    ...
    promptPartes.append(
        "3.  **DESCRIPCIÓN CONCISA**: En `descripcion`, un mensaje claro y breve para un commit (ej: 'Refactor: Mueve función `miUtilidad` de `a.php` a `lib/utils.php`').")
    promptPartes.append(
        "4.  **PARÁMETROS PRECISOS**: En `parametros_accion`, incluye SOLO los identificadores necesarios para que OTRO proceso (Paso 2) realice el cambio. Usa rutas RELATIVAS (separador `/`).")
    promptPartes.append(
        "    -   `mover_funcion`/`mover_clase`: `archivo_origen`, `archivo_destino`, `nombre_funcion`/`nombre_clase`. ¡Indica si el archivo destino debe crearse!")
    ...
    promptPartes.append(
        "    -   `crear_archivo`: `archivo` (ruta completa relativa), `proposito_del_archivo` (ej: 'Clase para gestionar conexiones a API externa X').")
    ...
    promptPartes.append(
        "6.  **FORMATO JSON ESTRICTO**: Responde ÚNICAMENTE con el JSON, sin explicaciones fuera del JSON.")
...
```""")
    ...
    if historialCambiosTexto:
        ...
        promptPartes.append(
            "¡IMPORTANTE: Analiza el historial para no repetir errores o acciones inútiles!")
    ...
    try:
        respuesta = modelo.generate_content(
            ...
            generation_config=genai.types.GenerationConfig(
                temperature=0.3),  # Un poco más determinista
        ...
        if not textoRespuesta:
            return None  # Error ya logueado en helper
        ...
        if sugerenciaJson and not all(k in sugerenciaJson for k in ["razonamiento", "accion_propuesta", "descripcion", "parametros_accion", "archivos_relevantes"]):
            log.error(
                f"{logPrefix} Respuesta JSON incompleta, faltan campos clave obligatorios. JSON: {sugerenciaJson}")
            return None
...
def ejecutarAccionConGemini(decisionParseada, contextoCodigoReducido):
    ...
    if not decisionParseada:
        log.error(
            f"{logPrefix} No se proporcionó la decisión detallada del Paso 1.")
        return None
    ...
    # Para contexto, aunque no se use directamente
    ...
    razonamiento_paso1 = decisionParseada.get(
        "razonamiento", "No proporcionado.")  # Para contexto interno
    archivos_relevantes_paso1 = decisionParseada.get(
        "archivos_relevantes", [])  # Para referencia

    # --- Prompt REVISADO para Paso 2 (Enfoque en EJECUCIÓN PRECISA) ---
    # (Dentro de la función ejecutarAccionConGemini en nucleo/analizadorCodigo.py)

    # --- Prompt REVISADO para Paso 2 (Enfoque en EJECUCIÓN PRECISA y SINTAXIS CORRECTA) ---
    promptPartes = []
    promptPartes.append("Eres un asistente de ejecución de refactorización ALTAMENTE PRECISO. Se ha tomado una decisión en un paso anterior. TU ÚNICA TAREA AHORA es ejecutarla SIN ERRORES, prestando MÁXIMA ATENCIÓN a la sintaxis del lenguaje y a las instrucciones.")
    promptPartes.append(
    ...
    promptPartes.append(
        "\n--- TUS INSTRUCCIONES DETALLADAS DE EJECUCIÓN (¡CRÍTICO SEGUIRLAS!) ---")
    promptPartes.append(
        "1.  **RECIBE**: Se te da el contenido ACTUAL de los archivos relevantes.")
    promptPartes.append(
        "2.  **EJECUTA**: Realiza la acción `{accion}` usando los `parametros_accion`.")
    promptPartes.append(
        "    -   **Si es `mover_funcion` o `mover_clase`**: Localiza la función/clase en `archivo_origen`. CÓPIALA EXACTAMENTE a `archivo_destino`. ELIMÍNALA COMPLETAMENTE de `archivo_origen`. Si `archivo_destino` existe, inserta el código copiado en una ubicación LÓGICA (ej: al final, antes del `?>` final si existe, o junto a funciones similares). **NO insertes etiquetas `<?` o `?>` si ya estás dentro de un bloque PHP existente en el destino.** Asegúrate de que no haya etiquetas PHP anidadas o innecesarias. Si `archivo_destino` NO existe, créalo comenzando con `<?` y luego el código copiado. Añade `// Movido automáticamente por IA desde {archivo_origen}` en el destino.")
    promptPartes.append("    -   **Si es `modificar_codigo_en_archivo`**: Aplica el cambio descrito en `descripcion_detallada_cambio` al archivo `archivo`. Sé preciso. NO introduzcas otros cambios. **NO añadas `<?` o `?>` innecesariamente.** Asegúrate de que la sintaxis PHP resultante sea válida. Añade `// Modificado automáticamente por IA` cerca del cambio.")
    promptPartes.append(
        "    -   **Si es `crear_archivo`**: Genera el contenido inicial para `archivo` basado en `proposito_del_archivo`. Asegúrate de que sea un archivo PHP válido comenzando con `<?`.")
    promptPartes.append(
        "    -   **Si es `eliminar_archivo` o `crear_directorio`**: Responde con el JSON de resultado vacío (sin `archivos_modificados`).")
    promptPartes.append(
        "3.  **DEVUELVE**: Genera el CONTENIDO COMPLETO Y FINAL para CADA archivo que haya sido modificado o creado. ¡NO incluyas archivos que no cambiaste!")
    promptPartes.append(
        "4.  **PRESERVA**: Mantén el resto del código INTACTO.")
    promptPartes.append("5.  **SINTAXIS PHP (¡MUY IMPORTANTE!)**: ")
    promptPartes.append(
        "    -   Usa SIEMPRE `<?` para abrir etiquetas PHP, NUNCA `<?php`.")
    promptPartes.append(
        "    -   Usa `<? echo` para salidas cortas si es consistente con el código existente.")
    promptPartes.append("    -   **¡REGLA CRÍTICA! NUNCA insertes `<?` dentro de un bloque de código PHP que ya esté abierto (`<? ... ?>`). NUNCA tengas `<? <? ... ?> ?>`.** Asegúrate de que las etiquetas PHP estén balanceadas y sean sintácticamente correctas en TODO el archivo final.")
    promptPartes.append(
        "    -   **Revisa CUIDADOSAMENTE tu salida para evitar errores de sintaxis PHP antes de generar el JSON.**")
    promptPartes.append("6.  **FORMATO JSON (¡MUY IMPORTANTE!)**: ")
    promptPartes.append("    -   El contenido de cada archivo PHP debe ser incluido como una **CADENA JSON VÁLIDA**. Esto significa que caracteres especiales dentro del código PHP (como `\"`, `\\`, saltos de línea `\\n`, etc.) DEBEN ser escapados correctamente (`\\\\`, `\\\"`, `\\\\n`).")
    promptPartes.append(
        "    -   **UN ERROR EN EL ESCAPADO DE LAS CADENAS PHP INVALIDARÁ TODA TU RESPUESTA JSON.** Presta máxima atención a esto.")
    promptPartes.append(
    ...
    promptPartes.append(
        "1.  **CONTENIDO COMPLETO FINAL**: Para cada archivo afectado, proporciona su contenido ÍNTEGRO y final.")
    promptPartes.append(
        "2.  **SOLO ARCHIVOS AFECTADOS**: Incluye únicamente los archivos cuyo contenido ha cambiado o que has creado.")
    promptPartes.append("3.  **FORMATO JSON ESTRICTO**: Responde ÚNICAMENTE con un JSON que mapea la ruta RELATIVA (separador `/`) del archivo a su nuevo contenido COMPLETO, asegurándote de que las cadenas de contenido estén correctamente escapadas.")
    # El ejemplo JSON se mantiene igual, pero las reglas anteriores lo refuerzan
    promptPartes.append("""
```json
{
  "tipo_resultado": "ejecucion_cambio",
  "archivos_modificados": {
    "ruta/relativa/archivo_modificado1.php": "<?php\\n/* NUEVO CONTENIDO COMPLETO DEL ARCHIVO 1 (CORRECTAMENTE ESCAPADO) */\\n// Modificado automáticamente por IA\\n?>",
    "ruta/relativa/archivo_creado.php": "<?php\\n/* CONTENIDO INICIAL DEL NUEVO ARCHIVO (CORRECTAMENTE ESCAPADO) */\\n?>"
    // ... incluir una entrada por CADA archivo que cambie o se cree
  }
}
```""")
    # Caso especial: acciones sin modificación de contenido (sin cambios)
    ...
    try:
        ...
        if resultadoJson and isinstance(resultadoJson.get("archivos_modificados"), dict):
            claves_recibidas = set(
                resultadoJson["archivos_modificados"].keys())
            claves_relevantes_esperadas = set(
                archivos_relevantes_paso1)  # Archivos que le dimos
            ...
            if archivos_inesperados:
                ...
                archivo_destino_nuevo = params.get("archivo_destino") if accion in ["mover_funcion", "mover_clase"] and params.get(
                    "archivo_destino") not in contextoCodigoReducido else None  # Aproximación
    ...
    try:
        ...
        if not textoRespuesta:
            ...
            block_reason = getattr(
                feedback, 'block_reason', 'Desconocido') if feedback else 'Desconocido'
            safety_ratings = getattr(
                feedback, 'safety_ratings', 'No disponibles') if feedback else 'No disponibles'
            finish_reason_cand = 'Desconocido'
            if respuesta.candidates:
                finish_reason_cand = getattr(
                    respuesta.candidates[0], 'finish_reason', 'Desconocido')

            log.error(
                f"{logPrefix} Respuesta de Gemini vacía. BlockReason: {block_reason}, FinishReason: {finish_reason_cand}, SafetyRatings: {safety_ratings}")
...
def _limpiarYParsearJson(textoRespuesta, logPrefix):
    ...
    if not textoLimpio.startswith("{") or not textoLimpio.endswith("}"):
        ...
        if json_start != -1 and json_end != -1 and json_start < json_end:
            ...
            log.warning(
                f"{logPrefix} Se extrajo JSON potencialmente incrustado en texto adicional.")
        else:
            log.error(
                f"{logPrefix} Respuesta de Gemini no parece contener JSON válido. Respuesta (limpia): {textoLimpio[:500]}...")
...
def _manejarExcepcionGemini(e, logPrefix, respuesta=None):
    ...
    if isinstance(e, google.api_core.exceptions.ResourceExhausted):
        ...
        log.error(
            f"{logPrefix} Error argumento inválido (InvalidArgument): {e}. ¿Contexto muy grande o prompt mal formado?")
        ...
        block_reason = getattr(feedback, 'block_reason',
                               'Desconocido') if feedback else 'Desconocido'
        safety_ratings = getattr(
            feedback, 'safety_ratings', 'No disponibles') if feedback else 'No disponibles'
        ...
        if respuesta and respuesta.candidates:
            ...
            finish_reason = getattr(
                candidate, 'finish_reason', 'Desconocida') if candidate else 'Desconocida'
            safety_ratings_cand = getattr(
                candidate, 'safety_ratings', None) if candidate else None
            if safety_ratings_cand:
                safety_ratings = safety_ratings_cand
        log.error(
            f"{logPrefix} Razón Bloqueo: {block_reason}, Razón Fin: {finish_reason}, Safety: {safety_ratings}")
    else:
        log.error(
        ...
        if respuesta:
            feedback = getattr(respuesta, 'prompt_feedback', None)
            if feedback:
                log.error(f"{logPrefix} Prompt Feedback: {feedback}")
